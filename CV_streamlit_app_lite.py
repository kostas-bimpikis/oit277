"""
CV Match Explorer — Privacy Edition (Lite)
==========================================

Identical to streamlit_app.py EXCEPT:
  - Student profiles are never displayed (neither own nor matches')
  - LLM explanations are instructed to avoid reproducing sensitive details
    (military/security clearances, specific deal/company names, etc.)

HOW TO RUN LOCALLY:
  1. pip install -r requirements.txt
  2. streamlit run streamlit_app_lite.py

DEPLOY TO STREAMLIT CLOUD:
  Same as the full version — just point to this file instead.
"""
...
# The LLM still receives full profiles so it can reason well,
# but is instructed NOT to reproduce sensitive details in its output.
...
PRIVACY RULES — your output must follow these strictly:
- Do NOT reproduce, quote, or reference specific sensitive details such as:
  military service, intelligence roles, security clearances, specific company
  names, deal names, client names, financial figures, or any other personally
...
with tab_individual:
    ...
    st.markdown(
        ...
        "🔒 <strong>Privacy mode:</strong> Individual profiles are not displayed. "
        "LLM explanations are generated using general terms only — no sensitive details are shown."
        "</div>",
        unsafe_allow_html=True,
    )
    ...
    for i, m in enumerate(my_matches):
        ...
        # Per-field breakdowns (scores only, no profile text)
        field_scores = {}
        for f in PROFILE_FIELDS:
            ...
            for j, (field_name, field_score) in enumerate(field_scores.items()):
                with cols[j]:
                    short_name = (field_name
                                  .replace("What I'm Looking For", "Goals")
                                  .replace("Professional Background", "Professional")
                                  .replace("Academic Background", "Academic")
    ...
    # ── LLM Explanation (replaces "Learn More" profile view) ──
    st.markdown("---")
    st.subheader("🤖 Get an LLM Explanation")
    st.caption("Select a match and ask GPT-4o-mini to explain the pairing — without revealing any sensitive profile details.")
    ...
    # Cross-algorithm comparison (scores only, no profile text)
    st.markdown("---")
...
# Footer
st.markdown("---")
st.caption("Built with Streamlit · Semantic matching powered by OpenAI embeddings · Privacy Edition")
